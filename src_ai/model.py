...
class Map:
    """ Defines the gameState as an abstract collection of chunks. """
    ...
    def __str__(self):
        """ Representation when the map is used as a string. """
        for chunk in self.contents:
            pass
            # Display position as occupied.
...
class Chunk:
    ...
    def __init__(self, x=0, y=0, width=16, height=16):
        ...
        self.width = width
        self.height = height
        self.contents = [['.' for _ in range(width)] for _ in range(height)]
    ...
    def __str__(self, out=""):
        ...
        for y, row in enumerate(self.contents):
            ...
            # Quadrant Row-Row Spacers
            out += f"◼ {40*' '}   ◼\n" if (y > 0 and y % 4 == 0) else ''
            for x, val in enumerate(row):
                # Pretty Hex Col
                out += f"◼  {hval(y)} " if x == 0 else ''

                # Quadrant Col-Col Spacers
                out += '   ' if (x > 0 and x % 4 == 0) else ' '
    ...
    def setTileXY(self, x, y, tile):
        ...
        self.contents[-y][x-1] = tile
        return

    def getTileXY(self, x, y):
        """ Gets the tile located at a standard (x, y) coordinate. """
        return self.contents[-y][x-1]
